	...
	var obj =  function (env, pkg, server, isSSL) {
		...
		me.hubService = function (cfg, incomeData) {
			...
			for(o in clusters) {
				_f[o] = (function(o) { 
					return function(cbk) {
						console.log('--hubProxy--send.A6 .---');
						console.log(o);
						var link_url = ((isSSL) ? 'https://' : 'http://') + o + 'AA/';
						
						var ioCSocket = ioClient.connect(link_url, {timeout:3000, 'connect_timeout': 3000});
						...
						ioCSocket.on('connect_failed', function(){
							console.log('--disco connect_failed---');
						    cbk(false);
						});
						...
						ioCSocket.on('reconnect_attempt', function(n){
							console.log(n);
						    cbk(false);
						});
						...
						ioCSocket.on('connect_timeout', (timeout) => {
							console.log('--disco connect_failed-jj--');
							 cbk(false);
						  // ...
						});
						/*
		...
		me.hubQuery = function (cfg, room) {
			...
			for(o in clusters) {
				_f[o] = (function(o) { 
					return function(cbk) {
						var ioCSocket = ioClient.connect(((isSSL) ? 'https://' : 'http://') + o + '/');
						ioCSocket.on('connect', function() {
							ioCSocket.emit('hubQuery', {room:  room, hub : o}, (data) => {});									
							ioCSocket.disconnect();
							cbk(true);
						});
						ioCSocket.on('connect_failed', function(){
