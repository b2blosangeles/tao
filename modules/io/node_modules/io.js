	...
	var obj =  function (env, pkg, server, isSSL) {
		...
		me.hubService = function (cfg, incomeData) {
			...
			for(o in clusters) {
				_f[o] = (function(o) { 
					return function(cbk) {
						var ioCSocket = ioClient.connect(((isSSL) ? 'https://' : 'https//') + o + '/');
		...
		me.io.on("connection", (socket) => {
			...
			socket.on('askUniqueId', function(){
				let s_no = (me.s_no > 1000000) ? 1 : me.s_no++;
				let tm = Math.floor(new Date().getTime() * 0.001);
				let unistr = (host + tm).toUnicode();
				me.io.to(socket.id).emit('uniqueId', unistr  + '-' + s_no );
			});
