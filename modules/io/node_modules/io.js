(function () {
	...
	var obj =  function (env, pkg, server) {
		...
		this.getCluster = function (host) {
			...
			let list = {}
			for(o in DS) {
				if ((host.match(/^comm([0-9]+)\_(dev)\.([a-z0-9]+)\.([a-z0-9]+)$/ig))
					(o.match(/^comm([0-9]+)\_(dev)\.([a-z0-9]+)\.([a-z0-9]+)$/ig))
				) { list[o] = DS[o]}
				
				if ((host.match(/^comm([0-9]+)\_(prod)\.([a-z0-9]+)\.([a-z0-9]+)$/ig))
					(o.match(/^comm([0-9]+)\_(prod)\.([a-z0-9]+)\.([a-z0-9]+)$/ig))
				) { list[o] = DS[o]}
				
				if ((host.match(/^comm([0-9]+)\_(qa)\.([a-z0-9]+)\.([a-z0-9]+)$/ig))
					(o.match(/^comm([0-9]+)\_(qa)\.([a-z0-9]+)\.([a-z0-9]+)$/ig))
				) { list[o] = DS[o]}
		...
		me.hubService = function (cfg, data) {
			let me = this, ips = _getServerIP();
			me.io.to(cfg.socketid).emit('hubService', {ip : ips, cfg : cfg, dns : me.getCluster(cfg.host) });		
		};
