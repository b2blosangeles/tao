(function () {
	var obj =  function (env, pkg, server, isSSL) {
		...
		this.ioApp = function(env, pkg, server, isSSL) {
			delete require.cache[env.root_path + '/modules/io/node_nodules/ioApp.js'];
			let ioAPP =  require(env.root_path + '/modules/io/node_nodules/ioApp.js');
			new ioAPP(env, pkg, server, isSSL);
		}
		this.getCluster = function (host, full) {
			let me = this, DS_m = {}, DS_d = {}, DS = {};	
			return this.ioApp(env, pkg, server, isSSL).getCluster(host, full);
			...
			CP.serial(_f, function(data) {
				...
				for (o in data.results) {
					if (typeof data.results[o] === 'object') {
						...
						/*
						for (var i = 0;  i < data.results[o].length; i++) {
							clients[data.results[o][i]] = 1;
						}*/
