(function () {
	...
	var obj =  function (env, pkg, server) {
		...
		this.runSocketService = function(cfg, income_data) {
			var me = this, outData = {};
			if (!cfg._code) {
				outData =  income_data
			} else {
						...
						pkg.fs.stat(p, function(err, stats) {
							 if (stats.isFile()) {
								...
								try {
									...
									var entity = new taskClass(pkg, me.envSite(env), io);
									entity.call();
								} catch(err) {
									pkg.fs.readFile(p, 'utf8', function(err, code) {
										if (!err) {
											try {
												...
												({require : require, pkg: pkg, env: me.envSite(env), io : io});
											} catch(err) {
												me.send500(err);
											}
										} else {
		...
		me.handleRequest = function (cfg, data) {
			let me = this;
		...
		me.io.on("connection", (socket) => {
			...
			socket.on('clientRequest', function(incomeData){
				...
				switch(incomeData.cmd) {
					case 'sendToRoom':
						if (!incomeData.room) {
							me.handleRequest(cfg, {_id: _id,  socket_id: socket.id,
								Err : 'Missing room'});
						} else {
					...
					case 'sendToSocket':
						...
						break;	
					default : 
						me.handleRequest(cfg, {_id: incomeData._id,  socket_id: socket.id,Err : 'Missing cmd'});
				}
			});
