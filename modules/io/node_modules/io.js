(function () {
	var obj =  function (env, pkg, server, isSSL) {
		...
		me.isAuthServer = function(host) {
			var TP = ['auth'], ENV = ['dev', 'prod', 'qa'];
			for (var i = 0; i < TP.length; i++) {
				for (var j = 0; j < ENV.length; j++) {
					var patt = new RegExp('^'+ TP[i] +'([0-9]+)\_' + ENV[j] + '\.([a-z0-9]+)\.([a-z]+)$', 'ig');
		...
		me.io.on("connection", (socket) => {
			...
			if me.isAuthServer(host) {
				delete require.cache[env.root_path + '/modules/io/node_modules/ioAuth.js'];
				let ioAuth =  require(env.root_path + '/modules/io/node_modules/ioAuth.js');
				try {
					new ioAuth(me, env, pkg, server, host, isSSL).onConnection(socket);
				} catch (err) {
