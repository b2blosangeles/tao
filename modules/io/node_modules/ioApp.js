(function () {
	var obj =  function (env, pkg, server, isSSL) {
		...
		this.serverRule = function(name, type, environment) {
			// var patt =  new RegExp('^'+ type +'([0-9]+)\_' + environment + '\.([a-z0-9]+)\.([a-z]+)$', 'ig');
			var patt = host.match(/^comm([0-9]+)\_dev\.([a-z0-9]+)\.([a-z0-9]+)$/ig;
			return (name.match(patt)) ? true : false;
		}
		this.getCluster = function (host) {
			...
			for(o in me.domainList()) {
				if (me.serverRule(host, 'comm', 'dev') && me.serverRule(o, 'comm', 'dev')) 
		...
		this.hubQuery = function (cfg, room, session_id, IO, callback) {
			let me = this, clusters =  me.getCluster(cfg.host);
