(function () {
	var obj =  function (env, pkg, server, isSSL) {
			...
			_f['self'] = function(cbk)  {
				IO.io.in(room).clients((err, clients) => {
					...
					for (var i = 0 ;  i < clients.length; i++) {
						if (IO.mapping[clients[i]]) {
							obj_clients[clients[i]] =  IO.mapping[clients[i]];
						} else {
		...
		me.onConnection = function (IO, socket) {
			IO.io.to(socket.id).emit('uniqueId', pkg.comm.getSERVERID() + '_' + socket.id.replace(/\/\#/g, ''));
			...
			socket.on('askUniqueId', function(){
				IO.io.to(socket.id).emit('uniqueId', pkg.comm.getUUID());
			});
			*/
			...
			socket.on('setClientId', function(id){
			    IO.mapping[socket.id] = 'CID_' + id;
			});			

			sequenceNumberByClient.set(socket, 1);
			socket.on("disconnect", () => {
				delete IO.mapping[socket.id];
