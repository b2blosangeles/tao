(function () {
	var obj =  function (IO, env, pkg, server, host, isSSL) {
		...
		me.hubLink = function(link_url, callback, err_callback) {
			...
			if (!me.ioPool[link_url]) {
				console.log('--CCC>>>---' + link_url);
				me.ioPool[link_url] = ioClient.connect(link_url, {reconnection: false, timeout:3000, 'connect_timeout': 3000});

				me.ioPool[link_url].on('connect', (function(me) {
					return function() {
					callback(me.ioPool[link_url], false);
					console.log('-KKKAAA>>>---' + link_url);
					console.log(me.ioPool[link_url].connected);
					}
				})(me));
				me.ioPool[link_url].on('connect_error', function(error){
				...
				me.ioPool[link_url].on('connect_timeout', (timeout) => {
					me.ioPool[link_url].disconnect();
					delete me.ioPool[link_url];
				...
				if (me.ioPool[link_url].connected) {
						console.log('-no need link CCC>>>---' + link_url);
							console.log(me.ioPool[link_url].connected);
							callback(me.ioPool[link_url],  true);
				} else {
