(function () {
	var obj =  function (env, pkg, server, isSSL) {
		...
		this.serverRule = function(N, type, ENV) {
			
			var patt = new RegExp('^'+ type +'([0-9]+)\_' + ENV + '\.([a-z0-9]+)\.([a-z]+)$', 'ig');
			return (patt.test(N[0]) && patt.test(N[1]));
		}
		this.getCluster = function (host) {
			...
			for(o in me.domainList()) {
				if (me.serverRule([host, o], 'comm', 'dev') 
					|| me.serverRule([host, o], 'comm', 'prod')
					|| (me.serverRule([host, o], 'comm', 'qa'))
				) { 
					 if (DS[o] !== DS[host]) {
