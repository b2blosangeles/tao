	...
	var obj =  function (IO, env, pkg, server, host, isSSL) {
		...
		me.isPipeServer = function(host) {
			...
			for (var i = 0; i < TP.length; i++) {
				for (var j = 0; j < ENV.length; j++) {
					// var patt = new RegExp('^'+ TP[i] +'([0-9]+)\_' + ENV[j] + '\.([a-z0-9]+)\.([a-z]+)$', 'ig');
					var patt = new RegExp('^'+ 'comm' +'([0-9]+)\_' + 'dev' + '\.([a-z0-9]+)\.([a-z]+)$', 'ig');
		...
		me.getCluster = function (host) {
			...
			let list = {}, ip_list = [];
			for(o in DS) {
				if (me.serverRule(o) && me.serverRule(host))  { 
					// if (DS[o] !== DS[host]) {
		...
		this.hubLink = function(link_url, callback, err_callback) {
			let me = this;
			if (!IO.ioPool[link_url] || !IO.ioPool[link_url].connected) {
				IO.ioPool[link_url] = ioClient.connect(link_url, {reconnection: false, timeout:3000, 'connect_timeout': 3000});

				IO.ioPool[link_url].on('connect', function() {
					callback(IO.ioPool[link_url], false);
				});
				IO.ioPool[link_url].on('connect_error', function(error){
					IO.ioPool[link_url].disconnect();
					delete IO.ioPool[link_url];
					err_callback();
				});

				IO.ioPool[link_url].on('connect_timeout', (timeout) => {
					IO.ioPool[link_url].disconnect();
					delete IO.ioPool[link_url];
					err_callback();
				});
			} else {
				callback(IO.ioPool[link_url], true);
			}
		}
		this.hubToAServer = function (incomeData) {
			let me = this;
			...
			if (CA.length === 2 && pkg.comm.isIp(CA[0])) {
				...
				me.hubLink(link_url, 
					function(client_socket, pool) {
						let sid = '/#' + CA[1];
						// client_socket.to(`${me._socket.id}`).emit('clientMessage', {
				//	hubProxy
						...
						// IO.io.to(`${me._socket.id}`).emit('clientMessage', {
							...
							data 		: {
								...
								POOL : me._socket.id + '--8888-->' + sid + '-' +  host,
								handshake : IO.domainList(),
								getCluster : me.getCluster(host),
								//me._socket.handshake
							}
						}); 
					},
					function() {
