(function () {
	var obj =  function (IO, env, pkg, _pool, server, host, isSSL) {
		...
		this.hubRoomCommand = function (room, incomeData) {
			let me = this, clusters =  IO.getCluster(host);
			...
			for(o in clusters) {
				_f[o] = (function(o) { 
					...
					IO.hubLink(link_url, host,
						function(client_socket, pool) {
							client_socket.emit('hubProxyCommand', {
								room : room,
								data : incomeData
							}); 
							cbk(true);
		...
		me.onConnection = function (socket) {
					...
					case 'leaveRoom':
						socket.leave(incomeData.room,  function() {
							me.hubQuery(incomeData.room, incomeData.session_id, IO,
								function(data) {
									/*
									me.deliveryMessage(socket.id, {
										code : 'roomClients', 
										clients : {}
									});
									*/
									me.hubRoomCommand(incomeData.room, {
										code : 'roomClients', 
										clients : (!data || !data.clients) ? {} : data.clients
										});
									/*
									me.deliveryMessage(incomeData.room, {
									code : 'roomClients', 
									clients : (!data || !data.clients) ? {} : data.clients
									});*/
								});
			...
			socket.on('hubProxyCommand', function(incomeData){
