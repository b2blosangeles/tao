(function () {
	var obj =  function (IO, env, pkg, server, host, isSSL) {
		...
		me.getCluster = function (host) {
			let me = this, DS = IO.domainList();
			if (!DS[host]) return [];
			let list = {}, ip_list = [];
			for(o in DS) {
				if (IO.serverRule(host) && IO.serverRule(o))  { 
					if (DS[o] !== DS[host]) {
		...
		me.hubLink = function(link_url, callback, err_callback) {
			let me = this;
			var ioClient = require(env.root_path + '/package/socket.io-client/node_modules/socket.io-client');
			if (!env._PLL) env._PLL = {}
			if (!env._IDD) env._IDD = new Date().getTime();
			console.log('====envIDD====>');
			console.log(env._IDD);
			//if (!me.ioPool[link_url] || !me.ioPool[link_url].connected) {
			if (!env._PLL[link_url]) {
				console.log('--CCC>>>---' + link_url);
				env._PLL[link_url] = ioClient.connect(link_url, {reconnection: false, timeout:3000, 'connect_timeout': 3000});

				env._PLL[link_url].on('connect', function() {
					callback(env._PLL[link_url], false);
					console.log('-KKKAAA>>>---' + link_url);
					console.log(env._PLL[link_url].connected);
						// me.ioPool[link_url].disconnect();
				});
				env._PLL[link_url].on('disconnect', function() {
					delete env._PLL[link_url];
					console.log('dis connect --- ' + link_url);
				});
				env._PLL[link_url].on('connect_error', function(error){
					link.disconnect();
					delete env._PLL[link_url];
					console.log('-KKKBBB>>>---' + link_url);
					err_callback();
				});

				env._PLL[link_url].on('connect_timeout', (timeout) => {
					link.disconnect();
					delete env._PLL[link_url];
					console.log('-KKKCCC>>>---' + link_url);
					err_callback();
				});
			} else {
				if (env._PLL[link_url].connected) {
						console.log('-no need link CCC>>>---' + link_url + '--ID==>' + env._PLL[link_url].id);
							console.log(env._PLL[link_url].connected);
							callback(env._PLL[link_url],  true);
				} else {
					console.log('SKIPPED ME ====>>' + link_url);
				}
		...
		this.hubQuery = function (room, session_id, IO, callback) {
			...
			let me = this, clusters =  me.getCluster(host);
		...
		me.callbackMessageA = function (data) {
			let me = this;
		...
		me.onConnection = function (socket) {
			...
			socket.on('clientRequest', function(incomeData){ 
				...
				switch(incomeData.cmd) {
					...
					case 'roomClients':
						console.log('MMMK==>0>' + incomeData.session_id);
						me.hubQuery(incomeData.room, incomeData.session_id, IO,
						    function(data) {
							console.log('MMMK==>1>'+ incomeData.session_id);
							me.callbackMessage({
								session_id : incomeData.session_id,
								data: {data:data, success : true}
							});
