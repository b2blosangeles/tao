(function () {
	var obj =  function (env, pkg, server, isSSL) {
		...
		me.sendClientMessage = function (IO, socket, data) {
			let me = this;
			IO.io.to(socket.id).emit('clientRequestResponse', {data : data});	
		};
		me.callbackMessage = function (IO, type, target, data) {
			let me = this;
			IO.io.to(target.id).emit('callbackMessage', {data : data});	
		};
					...
					case 'joinRoom':
						if (!incomeData.room) {
							...
							socket.join(incomeData.room, function() {
								me.hubQuery(cfg, incomeData.room, incomeData.session_id, IO);	
								/*
								me.sendResponse(IO, socket, {socket_id: socket.id, Success : true});
								*/
							});
						}
						break;
