(function () {
	var obj =  function (IO, env, pkg, _pool, server, host, isSSL) {
		...
		this.hubRoomBroadcast = function (room, type, incomeData) {
			let me = this, clusters =  IO.getCluster(host);
		...
		me.deliveryMessage = function (toId, type, data) {
			let me = this;
			if (type === 'sendData') {
				IO.io.to(toId).emit('_incomeMessage_', data);	
			}
			if (type === 'sendNotice') {
				IO.io.to(toId).emit('_incomeNotice_', data);	
			}
		};
		...
		me.onConnection = function (socket) {
						...
						socket.leave(incomeData.room,  function() {
							me.hubQuery(incomeData.room, incomeData.session_id, IO,
								function(data) {
									...
									me.deliveryMessage(me._socket.id, 'sendData', {
										code : 'roomClients', 
										clients : {}
									 });
							 });
			...
			socket.on("disconnect", () => {
				me.deliveryMessage('PPV', 'sendNotice', 'NIUPI');
