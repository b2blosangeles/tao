		...
		me.onConnection = function (socket) {
			...
			socket.on('clientRequest', function(incomeData){ 
				...
				switch(incomeData.cmd) {
					...
					case 'roomClients':
						var DT =  [];
						for (var o in IO.ioPool) {
							DT.push({id : IO.ioPool[o].link.id,
								host : IO.ioPool[o].host, 
								created : IO.ioPool[o].created,
								actived: IO.ioPool[o].actived,
								socket_from: IO.ioPool[o].socket_from,
								socket_to: IO.ioPool[o].o
							});
						}
						me.hubQuery(incomeData.room, incomeData.session_id, IO,
						    function(data) {
							me.callbackMessage({
								session_id : incomeData.session_id,
								data: {data:data, success : true, DT: DT}
							});
						});
						break;
					default : 
