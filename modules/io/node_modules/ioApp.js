(function () {
	var obj =  function (env, pkg, server, isSSL) {
		...
		this.hubToAServer = function (IO, uniqueId, cfg, incomeData) {
			let me = this;
			...
			if (CA.length === 2 && pkg.comm.isIp(CA[0])) {
				...
				incomeData.data.from = incomeData.from;
				incomeData.data.to = incomeData.to;
				incomeData.data.session_id = incomeData.session_id;
				incomeData.data.clientInfo = IO.mapping[uniqueId];
		...
		me.onConnection = function (IO, socket) {
			...
			socket.on('clientRequest', function(incomeData){ 
				 me._SN = (!me._SN || me._SN > 99999) ? 1 : (me._SN + 1);
				incomeData.session_id = new Date().getTime() + '.' + ((!incomeData.session_id) ? '' : incomeData.session_id)  + '.' + me._SN;
				switch(incomeData.cmd) {
						...
						if (!incomeData.room) {
							...
							socket.join(incomeData.room, function() {	
								IO.io.in(incomeData.room).clients((err, clients) => {
									...
									//	var uniqueId = me.CommIP  + '->' + socket.id.replace(/\/\#/g, '');
										IO.mapping[uniqueId] = (!incomeData || !incomeData.data || incomeData.data.clientInfo=== null) ? IO.mapping[uniqueId] : incomeData.data.clientInfo;
					...
					case 'sendToClient':
						...
						if (!incomeData.toClient) {
							...
							me.hubToAServer(IO, uniqueId, cfg, incomeData);
						}
						break;
					case 'leaveRoom':
					...
					//	});
						break;
					default : 
						me.sendResponse(IO, cfg, {socket_id: socket.id,Err : 'Missing cmd'});
				}
			});
