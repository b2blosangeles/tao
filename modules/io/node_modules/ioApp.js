			...
			if (CA.length === 2) {
				...
				ioCSocket.on('connect', function() {
					IO.io.to(`${me._socket.id}`).emit('clientMessage', {
						...
						data : '--room-3-' + link_url 
					});
					/*
					if (incomeData.data) {
						ioCSocket.io.to(`${me._socket.id}`).emit('sendToClient', {
						    session_id : incomeData.session_id,
				...
				/*
				IO.io.to(`${me._socket.id}`).emit('sendToClient', {
					session_id 	: incomeData.session_id,
					data: {
						room		: '--room-3-' + link_url 
					}
				});	
				*/
			}
							...
							socket.join(incomeData.room, function() {	
								IO.io.in(incomeData.room).clients((err, clients) => {
									...
									//	var uniqueId = me.CommIP  + '->' + socket.id.replace(/\/\#/g, '');
										...
										IO.io.to(incomeData.room).emit('roomMessage', {
											session_id:incomeData.session_id,
											data: incomeData.data,
											from: socket.id,
											to : incomeData.room
