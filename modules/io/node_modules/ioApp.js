	...
	var obj =  function (IO, env, pkg, server, host, isSSL) {
		...
		me.hubLink = function(link_url, callback, err_callback) {
			...
			if (!IO._PLL[link_url]) {
				...
				IO._PLL[link_url] = ioClient.connect(link_url, {reconnection: false, timeout:3000, 'connect_timeout': 3000});

				IO._PLL[link_url].on('connect', function() {
					callback(IO._PLL[link_url], false);
					console.log('-KKKAAA>>>---' + link_url);
					console.log(IO._PLL[link_url].connected);
						// me.ioPool[link_url].disconnect();
				});
				IO._PLL[link_url].on('disconnect', function() {
					delete me.ioPool[link_url];
					console.log('dis connect --- ' + link_url);
				});
				IO._PLL[link_url].on('connect_error', function(error){
					link.disconnect();
					delete IO._PLL[link_url];
					console.log('-KKKBBB>>>---' + link_url);
					err_callback();
				});

				IO._PLL[link_url].on('connect_timeout', (timeout) => {
					link.disconnect();
					delete IO._PLL[link_url];
